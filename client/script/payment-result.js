...
function renderSuccess({
  ...
  if (currentLang === "en") {
    ...
    if (subtitleEl)
      subtitleEl.innerHTML =
        'Thank you for choosing <strong style="color:var(--gold)">Celesté House</strong>.<br>A confirmation has been sent via email & SMS.';
  } else {
  ...
  if (document.getElementById("r-order-id"))
    document.getElementById("r-order-id").textContent = txnRef || "—";
  if (document.getElementById("r-amount"))
  ...
  if (document.getElementById("r-table"))
  ...
  if (actionsEl) {
    ...
    const printText =
      currentLang === "en" ? "Print Confirmation" : "In phiếu xác nhận";
...
function renderFailure(code, txnRef, currentLang) {
  ...
  if (currentLang === "en") {
    if (titleEl)
      titleEl.textContent = isCancel
        ? "Transaction Cancelled"
        : "Payment Failed";
    if (subtitleEl)
      subtitleEl.textContent = isCancel
  ...
  if (actionsEl) {
    ...
    const retryText =
      currentLang === "en" ? "↺ Retry Payment" : "↺ Thử Thanh Toán Lại";
    const backText =
      currentLang === "en" ? "← Back to Booking" : "← Quay lại đặt bàn";
...
function sendConfirmationEmail(params) {
  ...
  emailjs
    .send("service_opu2zvh", "template_jau8v18", params)
