...
export default function Checkout({ setPage, invoice, onPaymentSuccess }) {
  ...
  const isValidName = (name) => {
    ...
    // Check length between 2-100 characters
    if (trimmedName.length < 2 || trimmedName.length > 100) return false;
    // Allow letters (all languages including Vietnamese), spaces, hyphens, and apostrophes
    // Disallow numbers, @, #, $, %, ^, &, *, (, ), etc.
    return !/[0-9!@#$%^&*()_+\-=[\]{};:'"|,.<>?\\]/.test(trimmedName);
  };

  const isValidPhone = (phone) => {
    const phoneOnly = phone.trim().replace(/\s/g, "");
    // Valid Vietnamese phone: 10 digits starting with 0, or +84{9-11} digits
    ...
    <>
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    maxLength="100"
                    required
                    className={`flex-1 border rounded px-3 py-1.5 outline-none focus:border-blue-500 ${
                      customerName.trim() === ""
                        ? "border-gray-300"
                ...
                {customerName.trim() !== "" && !isValidName(customerName) && (
                  <p className="text-xs text-red-500 ml-24">
                    Tên phải từ 2-100 ký tự, chỉ chứa chữ cái và khoảng trắng
                  </p>
                )}
              </div>
              <div className="flex flex-col gap-1">
            ...
            <tbody>
              {rawItems.length > 0 ? (
                rawItems.map((item, index) => {
                  ...
                  const discount =
                    item.menuItem?.discount ?? item.discount ?? 0;
        ...
        <div className="mb-8 bg-gray-50 p-4 rounded border border-gray-100">
          ...
          {paymentMethod === "VNPAY" && (
            <div className="mt-6 pt-6 border-t border-gray-200 animate-fade-in-down">
              <div className="flex flex-col items-center justify-center bg-white p-6 rounded-xl border-2 border-dashed border-blue-300 w-fit mx-auto shadow-sm">
      ...
      {showInvoice && selectedOrder && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4">
          <div className="print-area bg-white shadow-2xl w-full max-w-xl overflow-hidden flex flex-col max-h-[90vh] font-mono border-4 border-gray-300">
            {/* HEADER */}
            <div className="text-center border-b-2 border-dashed border-gray-400 px-6 py-5">
              ...
              <p className="text-sm text-gray-600">Hotline: +84 123 456 789</p>

              <h3 className="text-xl font-bold mt-4 uppercase">
                Hóa đơn thanh toán
              </h3>

              <p className="text-sm mt-2">Mã HĐ: HD-{selectedOrder.id}</p>
            ...
            <div className="p-6 overflow-y-auto">
              ...
              {/* DANH SÁCH MÓN */}
              <h4 className="font-bold text-gray-800 mb-4">Danh sách món ăn</h4>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {rawItems.map((item, idx) => {
                      ...
                      const itemTotal =
                        item.subtotal ?? effectivePrice * itemQty;
