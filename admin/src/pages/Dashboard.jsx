...
export default function DashBoard({
  ...
  const reservationsByTableId = useMemo(() => {
    return reservations.reduce((map, reservation) => {
      if (reservation.status === "CANCELLED") return map;
            ...
            tables.map((table) => {
              ...
              const minutesLeft = upcomingReservation
                ? Math.max(
                    0,
                    Math.ceil(
                      (new Date(upcomingReservation.reservationTime) - new Date()) /
                        60000,
                    ),
                  )
                : null;
                    ...
                    <div className="flex flex-col h-full justify-between">
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span
                            className={`rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.15em] ${invoice ? "bg-red-200 text-red-800" : "bg-yellow-200 text-yellow-800"}`}
                          >
                            {statusLabel}
                          </span>
                        </div>
            ...
            {activeReservation && !activeInvoice ? (
              <div className="space-y-3">
                <div className="rounded-2xl bg-yellow-50 p-4 text-sm text-slate-700">
                  ...
                  <p>
                    Còn {Math.max(0, Math.ceil((new Date(activeReservation.reservationTime) - new Date()) / 60000))} phút tới giờ đặt.
                  </p>
                </div>
