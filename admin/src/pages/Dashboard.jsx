...
export default function DashBoard({
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [tablesData, reservationsData] = await Promise.all([
        parseSafely(tablesRes),
        parseSafely(reservationsRes),
      ]);
 
      setTables(tablesData || []);
      const updatedReservations = await autoCompleteWithData(reservationsData || []);
  ...
  const reservationsByTableId = useMemo(() => {
    ...
    return reservations.reduce((map, reservation) => {
      if (
        ...
        return map;
      // Bỏ qua reservation đã qua giờ (dù DB chưa cập nhật)
      if (reservation.reservationTime && new Date(reservation.reservationTime) <= now)
        return map;
  ...
  const handleCardClick = (table) => {
    ...
    // avoid shadowing with the `isAvailable` used in render
    const canCreateNew =
  ...
  const handleWelcomeGuest = async () => {
    ...
    // Lưu ý: Đổi `activeReservation.id` thành đúng tên trường ID của bạn (ví dụ: reservationId) nếu cần
    const resId = activeReservation.id || activeReservation.reservationId;
    ...
    try {
      ...
      // Sau khi khách vào, bạn có thể tự động gọi hàm tạo hóa đơn mới cho bàn đó
      ...
      console.error("Lỗi khi đón khách:", err);
    }
  };
    ...
    <div className="p-4 sm:p-6 lg:p-8 bg-[#FAF8F5] min-h-screen text-[#332A21] font-sans antialiased flex-1">
      ...
      <div className="mb-6 sm:mb-10 flex flex-col gap-4 sm:gap-6 pb-4 sm:pb-6 border-b border-[#EFEBE4]">
        <div>
          <span className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-[#C49A6C] flex items-center gap-1.5">
            <Sparkles size={12} className="text-[#C49A6C]" /> Sảnh tiệc & Phòng
            ăn chính
          </span>
      ...
      {loading ? (
        <div className="text-center py-24 flex flex-col items-center justify-center gap-2">
          ...
          <p className="text-xs tracking-wider uppercase font-bold text-[#A39688]">
            Đang bài trí bàn ăn...
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
          {tables.length > 0 ? (
            tables.map((table, index) => {
              ...
              let iconVisual = (
                <Utensils size={18} className="text-[#10B981]" />
              );
              ...
              if (invoice) {
                cardBg =
                  "bg-gradient-to-br from-[#FFF8F6] to-white border-[#FADCD5]";
                ...
                cardBg =
                  "bg-gradient-to-br from-[#F0FDFA] to-white border-[#CCFBF1]";
              ...
              return (
                ...
                >
                  <div className="flex flex-col justify-between h-full space-y-3 sm:space-y-4 min-w-0 flex-1 mr-3">
                    <div>
                      <span
                        className={`text-[10px] font-extrabold px-2.5 py-0.5 rounded-full border uppercase tracking-wider inline-block truncate max-w-full ${stateBadge}`}
                      >
                        {stateText}
                      </span>
                    ...
                    <div className="pt-1 sm:pt-2">
                      {invoice ? (
                        ...
                        <p className="text-xs text-[#059669] font-bold italic">
                          Đang trống lịch
                        </p>
                      )}
                    </div>
                  ...
                  <div
                    className={`relative flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-tr from-[#FAF8F5] to-white border border-[#ECE7E0] shadow-sm flex-shrink-0 transition-all duration-300 ${glowClass}`}
                  >
                    <div
                      className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full border flex items-center justify-center transition-transform duration-500 group-hover:scale-110 ${centerCircleBg}`}
                    >
                      {iconVisual}
                    </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div
            className="fixed inset-0 bg-[#1A130E]/40 backdrop-blur-md"
            onClick={closeModal}
          ></div>
              ...
              <div className="space-y-3 sm:space-y-4">
                <div className="rounded-2xl bg-[#E6FDF9] border border-[#99F6E4] p-4 text-left">
                  ...
                  <p className="text-xs text-[#726456] mt-1 leading-relaxed font-medium">
                    Khách hàng đã book bàn này. Thời gian dự kiến có mặt tại nhà
                    hàng sau khoảng{" "}
                    <span className="font-extrabold text-[#1A130E]">
                      {Math.max(
                        0,
                ...
                <div className="flex gap-3 pt-1">
                  <button
                    ...
                    onClick={handleWelcomeGuest} // <-- Gắn hàm vào đây
                  >
                    Đón khách vào
                  </button>
                  <button
                    ...
                    onClick={handleCancelReservation} // <-- Gắn hàm vào đây
                  >
                    Hủy đặt hẹn
                  </button>
                </div>
