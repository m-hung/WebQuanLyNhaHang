...
export default function Categories() {
  ...
  const handleSave = async () => {
    if (!formData.nameVi.trim())
      return alert("Vui lòng nhập tên danh mục (Tiếng Việt)");
    if (!formData.nameEn.trim())
      return alert("Vui lòng nhập tên danh mục (Tiếng Anh)");
      ...
      <div className="mb-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 pb-6 border-b border-[#EFEBE4]">
        <div>
          <span className="text-[10px] font-extrabold uppercase tracking-[0.25em] text-[#C49A6C] flex items-center gap-1.5">
            <Sparkles size={12} className="text-[#C49A6C]" /> Phân loại thực đơn
            nhà hàng
          </span>
      ...
      <div className="space-y-4">
        <div className="hidden lg:flex items-center px-8 py-3 text-[10px] font-black text-[#A39688] uppercase tracking-[0.15em] border-b border-[#EFEBE4]">
          <div className="w-5/12 flex items-center gap-2">
            <Layers size={12} /> Tên danh mục (Tiếng Việt)
          </div>
          <div className="w-4/12 flex items-center gap-2">
            <Globe size={12} /> Tên phiên dịch (English)
          </div>
          <div className="w-[15%] text-center flex items-center justify-center gap-1">
            <Eye size={12} /> Trạng thái
          </div>
          ...
          categories.map((cat, index) => (
            ...
            >
              <div className="lg:w-5/12">
                <span className="lg:hidden text-[9px] font-bold text-[#C49A6C] block uppercase mb-1">
                  Tiếng Việt
                </span>
                {/* Thay thế thành font-medium và tracking-wide nhã nhặn */}
                <span className="text-base font-medium text-[#1A130E] tracking-wide">
                  {cat.nameVi}
                </span>
              </div>
              ...
              <div className="lg:w-4/12">
                ...
                <span className="text-sm text-[#726456] font-semibold">
                  {cat.nameEn}
                </span>
              </div>

              <div className="lg:w-[15%] flex lg:justify-center items-center gap-3 lg:gap-0">
                <span className="lg:hidden text-[9px] font-bold text-[#C49A6C] uppercase">
                  Hiển thị:
                </span>
                <button
          ...
          <div className="relative bg-white rounded-[40px] shadow-2xl w-full max-w-md overflow-hidden border border-[#ECE7E0] animate-modal-scale">
            ...
            <div className="p-6 space-y-6">
              <div>
                ...
                {(formData.nameVi || formData.nameEn) && (
                  <div className="mt-3 flex flex-wrap gap-2 text-[10px] font-bold uppercase tracking-wide">
                    {formData.nameVi && (
                      <span className="bg-[#FCEEEB] text-[#E07A5F] border border-[#F7D2C9] px-2 py-1 rounded-lg">
                        🇲🇳 {formData.nameVi}
                      </span>
                    )}
                    {formData.nameEn && (
                      <span className="bg-[#FAF8F5] text-[#726456] border border-[#EFEBE4] px-2 py-1 rounded-lg">
                        🇬🇧 {formData.nameEn}
                      </span>
                    )}
                  </div>
              ...
              <div className="flex items-center justify-between p-4 bg-[#FAF8F5] rounded-2xl border border-[#EFEBE4]">
                <div>
                  <span className="text-xs font-bold text-[#1A130E] uppercase tracking-wider block">
                    Kích hoạt hiển thị
                  </span>
                  <span className="text-[10px] text-[#A39688] font-semibold block mt-0.5">
                    Cho phép hiển thị trên thực đơn của khách
                  </span>
                </div>
                <button
                  type="button"
                ...
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-all duration-300 ${formData.active ? "translate-x-6" : "translate-x-1"}`}
                  />
                </button>
              </div>
            </div>
