    ...
    <div className="al-lang-tab-wrap">
      {[
        ...
        <button
          key={k}
          type="button"
          className={`al-lang-tab ${active === k ? "active" : ""}`}
          onClick={() => onChange(k)}
        >
          {label}
        </button>
...
function Modal({ mode, blog, onClose, onSaved }) {
  ...
  const handleSubmit = async () => {
    if (
      ...
      setError(
        "Vui lòng điền đầy đủ tiêu đề, nội dung (cả 2 ngôn ngữ) và tên tác giả.",
      );
      return;
    }
    setLoading(true);
    ...
    <div className="al-modal-overlay al-fade-in">
      <div className="al-modal">
          ...
          >
            ...
            <div>
              <label className="al-label">
                Tác giả <span style={{ color: "#C49A6C" }}>*</span>
              </label>
              <input
                name="authorName"
            ...
            >
              {langTab === "vi" ? (
                <>
                  ...
                  <div>
                    <label className="al-label">
                      Nội dung (VI) <span style={{ color: "#C49A6C" }}>*</span>
                    </label>
                    <textarea
                      name="contentVi"
                ...
                <>
                  ...
                  <div>
                    <label className="al-label">
                      Content (EN) <span style={{ color: "#C49A6C" }}>*</span>
                    </label>
                    <textarea
                      name="contentEn"
        ...
        <div className="al-modal-ft">
          <button className="al-btn-outline" onClick={onClose}>
            Hủy bỏ
          </button>
          <button
            className="al-btn-gold"
...
function ArticleCard({ article, onEdit, onDelete, onToggle, delay }) {
  ...
  return (
    <div
      className={`al-card al-fade-up ${!article.active ? "inactive" : ""} d${Math.min(delay, 10)}`}
    ...
    >
      ...
      <div className="al-thumb-wrap">
        ...
        <div
          className="al-thumb-placeholder"
          style={{ display: article.imgUrl ? "none" : "flex" }}
        >
          <BookOpen size={32} />
        </div>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 5 }}>
          <span className={`al-lang ${article.titleVi ? "vi" : "off"}`}>
            <Globe size={9} />
            VI
          </span>
          <span className={`al-lang ${article.titleEn ? "en" : "off"}`}>
            <Globe size={9} />
            EN
          </span>
          <span
            style={{ marginLeft: "auto", fontSize: 10.5, color: "#B5A080" }}
          >
            #{article.blogId}
          </span>
        </div>
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
          ...
          <span style={{ fontSize: 10, color: "#C0A880" }}>
            {formatDate(article.createdAt)}
          </span>
        </div>
        <div style={{ display: "flex", gap: 5 }}>
          <button
            className="al-icon-btn tog"
            title={article.active ? "Ẩn bài" : "Hiện bài"}
            onClick={() => onToggle(article)}
          >
            {article.active ? <EyeOff size={13} /> : <Eye size={13} />}
          </button>
          <button
            className="al-icon-btn edit"
            title="Chỉnh sửa"
          ...
          <button
            className="al-icon-btn del"
            title="Xóa"
            onClick={() => onDelete(article.blogId)}
          >
            <Trash2 size={13} />
          </button>
        </div>
...
function ArticleList() {
  ...
  const handleToggleActive = async (blog) => {
    try {
      ...
      const saved = await res.json();
      setArticles((prev) =>
        prev.map((a) => (a.blogId === saved.blogId ? saved : a)),
      );
    } catch {
  ...
  return (
    <div
      ...
      style={{
        minHeight: "100vh",
        background: "linear-gradient(160deg,#FDFAF4,#F5EDDF)",
        padding: "28px 24px",
      }}
    >
      <style>{STYLES}</style>
        ...
        <div className="al-modal-overlay al-fade-in">
          <div
            style={{
              ...
              padding: "28px 26px",
              textAlign: "center",
              boxShadow: "0 30px 80px rgba(60,30,10,.22)",
              animation: "al-scaleIn .3s ease",
            }}
          >
            <div
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: 10 }}>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 12,
                  border: "none",
                  background: "linear-gradient(135deg,#DC3C3C,#B02020)",
                  color: "#fff",
                  fontWeight: 600,
                  fontSize: 12.5,
                  cursor: "pointer",
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
            ...
            {[
              {
                ...
                bg: "rgba(196,154,108,.09)",
                color: "#7A5A2A",
                border: "rgba(196,154,108,.22)",
              },
              {
                label: `${totalVisible} đang hiện`,
                bg: "rgba(40,180,80,.07)",
                color: "#1A8040",
                border: "rgba(40,180,80,.2)",
              },
              {
                label: `${totalHidden} đang ẩn`,
                bg: "rgba(0,0,0,.05)",
                color: "#666",
                border: "rgba(0,0,0,.1)",
              },
              {
                label: `${totalBilingual} song ngữ`,
        ...
        >
          <span style={{ fontSize: 12.5, color: "#9A8068" }}>
            {loading
              ? "Đang tải..."
              : `Hiển thị ${paginatedArticles.length} / ${articles.length} bài viết`}
          </span>
        ...
        {loading ? (
          ...
          >
            ...
            <p style={{ fontSize: 13, color: "#B5A080" }}>
              Đang tải bài viết...
            </p>
          </div>
        ) : paginatedArticles.length === 0 ? (
          <div
            style={{ textAlign: "center", padding: "64px 0", color: "#B5A080" }}
          >
            <BookOpen
              size={36}
            ...
            <p style={{ fontSize: 12, marginTop: 4, opacity: 0.7 }}>
              Hãy thêm bài viết đầu tiên của nhà hàng!
            </p>
          </div>
        ) : (
        ...
        {pageCount > 1 && (
          ...
          >
            <span style={{ fontSize: 12, color: "#9A8068" }}>
              Trang <strong style={{ color: "#5A3E20" }}>{currentPage}</strong>{" "}
              / <strong style={{ color: "#5A3E20" }}>{pageCount}</strong>
            </span>
            <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
              ...
              {Array.from({ length: pageCount }, (_, i) => i + 1).map((p) => (
                <button
                  key={p}
                  className={`al-pg-btn ${currentPage === p ? "active" : ""}`}
