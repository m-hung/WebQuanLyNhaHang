...
function ProductCard({ product, onAdd, getEffectivePrice, cartQty }) {
  ...
  return (
      ...
      style={{
        ...
        border:
          cartQty > 0
            ...
            : "1.5px solid rgba(196,154,108,0.12)",
        borderRadius: 16,
        overflow: "hidden",
        cursor: "pointer",
    ...
    >
      ...
      {cartQty > 0 && (
        <div
          className={cartQty > 0 ? "cart-badge-pop" : ""}
          style={{
            position: "absolute",
            top: 8,
            right: 8,
          ...
          <div>
            ...
            <p
              style={{
                fontSize: 13,
                fontWeight: 700,
                color: hasDiscount ? "#E8514A" : "#C49A6C",
              }}
            >
    ...
    >
      ...
      >
        <button
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.background = "rgba(196,154,108,0.15)";
...
export default function Pos({
  ...
  const getEffectivePrice = (item) =>
    Math.max(0, (item.price || 0) - (item.discount || 0));
    ...
    allIds.forEach((id) => {
      ...
      if (changeQty !== 0) {
        const effectivePrice = getEffectivePrice(
          currentMap[id] || originalMap[id],
        );
        changes.push({
          itemId: id,
          nameVi:
            currentMap[id]?.nameVi || originalMap[id]?.nameVi || "Unknown",
  ...
  const totalChangeAmount = cartChanges.reduce(
    (s, c) => s + c.changeQty * c.price,
    0,
  );
  const calculatedTotal = cart.reduce(
    (s, i) => s + getEffectivePrice(i) * i.qty,
    0,
  );
  ...
  const handleSave = () => {
    ...
    if (selectedTableObj && !editingInvoice) {
      fetch(`/api/tables/${selectedTableObj.tableId}/status`, {
        method: "PUT",
      ...
      if (onSaveInvoice) {
        ...
        const syncCartItems = () =>
          fetch(`/api/order-items/order/${editingInvoice.id}`, {
            method: "DELETE",
          }).then(() =>
            Promise.all(
              cart.map((item) =>
        ...
        const doUpdate = () =>
          fetch(`/api/orders/${editingInvoice.id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
        ...
        if (oldTableId !== finalTableId && newTableObj) {
          fetch(`/api/tables/${oldTableId}/status`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ status: "Available" }),
          })
            .then(() =>
              fetch(`/api/tables/${finalTableId}/status`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ status: "Occupied" }),
              }),
  ...
  return (
    ...
    >
        ...
        {isTableFull && (
          <div
            style={{
              ...
              border: "1px solid rgba(232,81,74,0.25)",
              borderRadius: 10,
              padding: "6px 14px",
              color: "#FF7A74",
              fontSize: 12,
      ...
      {/* ── Main Content ─────────────────────────────────────────── */}
      <div
        style={{ display: "flex", flex: 1, alignItems: "flex-start", gap: 0 }}
      >
        {/* ═══ LEFT PANEL — Menu ═══════════════════════════════════ */}
        <div className="pos-left-panel pos-scrollbar">
          ...
          >
            <button
              className={
                selectedCategory === "Tất cả" ? "cat-pill-active" : "cat-pill"
              }
          ...
          <div style={{ paddingRight: 4, paddingBottom: 24 }}>
            {loading ? (
              ...
              >
                ...
                <p style={{ color: "#B0A090", fontSize: 14, fontWeight: 500 }}>
                  Không tìm thấy món
                </p>
              </div>
            )}
        ...
        <div
          className={`pos-cart-panel pos-scrollbar${cartOpen ? " open" : ""}`}
        >
          {/* Panel header */}
          <div
            style={{
          ...
          >
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <h2
                  className="serif"
                  style={{ color: "#2C1E14", fontSize: 18, fontWeight: 700 }}
                >
                  Hóa đơn
                </h2>
              </div>
          ...
          <div style={{ flex: 1, padding: "4px 20px" }}>
            {cart.length > 0 ? (
              ...
              <div style={{ padding: "40px 0", textAlign: "center" }}>
                ...
                />
                <p style={{ color: "#C0A880", fontSize: 13, fontWeight: 500 }}>
                  Chưa có món nào
                </p>
                <p style={{ color: "#D0C8B8", fontSize: 11, marginTop: 4 }}>
            ...
            {editingInvoice && cartChanges.length > 0 && (
              ...
              >
                ...
                >
                  <span
                    style={{ fontSize: 11, fontWeight: 700, color: "#7A5430" }}
                  >
                    Tổng đổi:
                  </span>
                  <span
                    style={{
                      fontSize: 12,
                      fontWeight: 700,
                      color: totalChangeAmount >= 0 ? "#27855A" : "#E8514A",
                    }}
                  >
                    {totalChangeAmount > 0 ? "+" : ""}
          ...
          >
            ...
            >
              ...
              >
                ...
                <span
                  style={{ fontSize: 13, color: "#C49A6C", fontWeight: 600 }}
                >
                  VNĐ
                </span>
              </span>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button
                ...
                onMouseLeave={(e) =>
                  (e.currentTarget.style.background = "rgba(196,154,108,0.08)")
                }
              >
      ...
      {/* ── Mobile Cart Overlay ── */}
      <div
        className={`pos-cart-overlay${cartOpen ? " open" : ""}`}
        onClick={() => setCartOpen(false)}
      />
    </div>
  );
