...
export default function Pos({
    ...
    useEffect(() => {
        ...
        // 3. GỌI API LẤY DANH SÁCH THU NGÂN TỪ BẢNG USERS
        fetch("http://localhost:8080/api/users")
            .then((res) => res.json())
            .then((data) => {
                // Lọc ra các user có role CASHIER và trạng thái ACTIVE
                ...
                // Nếu đang chỉnh sửa hóa đơn thì tự động hiển thị lại tên Thu ngân của hóa đơn đó
    ...
    const filteredProducts = products.filter((product) => {
        const productName = String(product?.nameVi || product?.name || "",).toLowerCase();
        ...
        const matchCategory = selectedCategory === "Tất cả" || (product?.category && productCatName === selectedCategory);
    ...
    const handleAddToCart = (product) => {
        setCart((prevCart) => {
            ...
            if (existingItem) {
                return prevCart.map((item) => item.itemId === product.itemId ? {...item, qty: item.qty + 1} : item,);
            } else {
                return [...prevCart, {
                    ...product,
    ...
    const getCartChanges = () => {
        ...
        const allItemIds = new Set([...Object.keys(originalMap), ...Object.keys(currentMap),]);
        ...
        allItemIds.forEach((itemId) => {
            ...
            const effectivePrice = getEffectivePrice(currentMap[itemId] || originalMap[itemId],);

            if (changeQty !== 0) {
                changes.push({
                    itemId: itemId,
                    nameVi: currentMap[itemId]?.nameVi || originalMap[itemId]?.nameVi || "Unknown",
    ...
    const totalChangeAmount = cartChanges.reduce((sum, change) => sum + change.changeQty * change.price, 0,);

    const calculatedTotal = cart.reduce((sum, item) => sum + getEffectivePrice(item) * item.qty, 0,);

    const isCartEmpty = cart.length === 0;
    const isTableFull = availableTables.length === 0 && !editingInvoice;
    const isDisablePay = isCartEmpty || isTableFull;

    return (<div className="bg-white p-4 min-h-full rounded-lg shadow flex flex-col">
        ...
        <div className="flex flex-col lg:flex-row gap-6 flex-1">
            ...
            <div className="lg:w-2/3 flex flex-col h-full">
                <div className="flex flex-wrap items-center justify-between mb-4 gap-2">
                    <div className="flex gap-2 flex-wrap">
                        <button
                            className={`px-4 py-2 rounded shadow-sm transition whitespace-nowrap ${selectedCategory === "Tất cả" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-blue-500 hover:text-white"}`}
                        ...
                        {categories.map((category) => {
                            ...
                            return (<button
                                ...
                                className={`px-4 py-2 rounded transition whitespace-nowrap flex gap-1 ${selectedCategory === catNameVi ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-blue-500 hover:text-white"}`}
                                onClick={() => setSelectedCategory(catNameVi)}
                            >
                                <span>{catNameVi}</span>
                                {catNameEn && (<span
                                    className={selectedCategory === catNameVi ? "text-blue-200" : "text-gray-400"}
                                >
                        / {catNameEn}
                      </span>)}
                            </button>);
                ...
                <div
                    className="flex-1 bg-gray-50/50 p-4 rounded shadow-inner border border-gray-100 overflow-y-auto max-h-[calc(100vh-220px)] custom-scrollbar">
                    {filteredProducts.length > 0 ? (
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                            ...
                            >
                                <div
                                    className="h-28 bg-gray-100 rounded mb-3 flex items-center justify-center text-gray-400 text-sm overflow-hidden">
                        ...
                        </div>) : (<div className="h-full flex flex-col items-center justify-center text-gray-400">
            ...
            <div className="lg:w-1/3 border-t lg:border-t-0 lg:border-l lg:pl-6 pt-4 lg:pt-0 flex flex-col h-full">
                ...
                <div className="flex-1 flex flex-col gap-4 min-h-0 mb-4">
                    {/* BẢNG GIỎ HÀNG CHÍNH */}
                    <div
                        className="flex-1 border border-gray-200 rounded-lg flex flex-col overflow-hidden bg-white shadow-sm min-h-[200px]">
                        <div className="overflow-y-auto custom-scrollbar flex-1">
                            <table className="w-full text-left text-sm">
                                ...
                                {cart.length > 0 ? (cart.map((item, index) => (<tr
                                ...
                                {item.qty}
                                    ...
                                    <td className="p-3 text-right font-bold text-blue-600 whitespace-nowrap">
                                        {(getEffectivePrice(item) * item.qty).toLocaleString()}
                                        đ
                                    </td>
                    ...
                    {editingInvoice && cartChanges.length > 0 && (<div
                        className="border border-orange-200 rounded-lg flex flex-col overflow-hidden bg-white shadow-sm max-h-56 shrink-0">
                        <div
                            className="bg-orange-100 px-3 py-2 text-sm font-bold text-orange-800 border-b border-orange-200 flex justify-between items-center">
                    ...
                    {cartChanges.length} mục
                        ...
                        <div
                            className="bg-orange-50 px-3 py-2 text-sm font-bold border-t border-orange-200 flex justify-between items-center">
                            <span className="text-orange-800">TỔNG TIỀN ĐỔI:</span>
                            <span
                                className={totalChangeAmount >= 0 ? "text-green-600" : "text-red-500"}
                            >
                    {totalChangeAmount > 0 ? "+" : ""}
              ...
              {calculatedTotal.toLocaleString()} VNĐ
                ...
                <div className="flex gap-3 mt-auto shrink-0">
                    <button
                        className={`flex-1 py-3.5 rounded-lg font-bold transition shadow-sm ${!isDisablePay ? "bg-red-500 hover:bg-red-600 text-white" : "bg-gray-300 text-gray-500 cursor-not-allowed"}`}
                        disabled={isDisablePay}
                        onClick={() => {
                            // KIỂM TRA: NẾU CHƯA CHỌN THU NGÂN THÌ BẬT CHỮ ĐỎ VÀ CHẶN LƯU
                            if (!selectedCashier) {
                            ...
                            if (selectedTableObj && !editingInvoice) {
                                fetch(`http://localhost:8080/api/tables/${selectedTableObj.tableId}/status`, {
                                    ...
                                    .catch((err) => {
                                        ...
                                        // SỬA: Hiển thị lỗi ra UI thay vì alert()
                                ...
                                if (onSaveInvoice) {
                                    ...
                                    const syncCartItems = () => {
                                        return fetch(`http://localhost:8080/api/order-items/order/${editingInvoice.id}`, {
                                            method: "DELETE",
                                        },).then(() => {
                                            const itemPromises = cart.map((item) => {
                                                const orderItemPayload = {
                                    ...
                                    if (oldTableId !== finalTableId && newTableObj) {
                                        fetch(`http://localhost:8080/api/tables/${oldTableId}/status`, {
                                            ...
                                            .then(() => {
                                                return fetch(`http://localhost:8080/api/tables/${finalTableId}/status`, {
                                                    method: "PUT",
                                                    headers: {"Content-Type": "application/json"},
                                                    body: JSON.stringify({status: "Occupied"}),
                                                },);
                                            ...
                                            .then(() => {
                                                return fetch(`http://localhost:8080/api/orders/${editingInvoice.id}`, {
                                                    method: "PUT",
                                                    headers: {"Content-Type": "application/json"},
